...
// Detects a sharp corner by measuring how fast error is growing (dError/dt).
// When triggered, correction is boosted and speed is cut BEFORE error peaks —
// this is what catches 90° corners that the normal PID reacts to too late.
//
// CORNER_SENSITIVITY: min |dError/loop| to trigger corner mode.
//   300-600 works for most 90° tracks at moderate speed. 0 = disabled.
int   CORNER_SENSITIVITY  = 0;
// CORNER_BOOST: Kp multiplier during corner mode (1.0=off, 2.0-3.0=recommended).
float CORNER_BOOST        = 2.0f;
// CORNER_DURATION_MS: how long corner boost stays on after trigger (ms).
int   CORNER_DURATION_MS  = 200;
// CORNER_SPEED_PCT: speed as % of BASE_SPEED while corner boost is active.
//   30-50% cuts speed hard the moment the corner is detected.
int   CORNER_SPEED_PCT    = 50;

// ── Corner state (runtime only, not persisted) ────────────
...
// ── End-zone detection ───────────────────────────────────
// The finish line is a wide black square that covers all 6 sensors at once.
// On a normal line only 1-2 sensors are active — so ≥5 active simultaneously
// is an unambiguous end-zone signature.
//
// END_ZONE_ENABLE      : master on/off switch.
// END_ZONE_MIN_SENSORS : how many sensors must read above threshold (4-6).
//                        5 is safe; 6 requires every sensor to trigger.
// END_ZONE_CONFIRM_MS  : how long the condition must hold before stopping (ms).
//                        50-150 ms filters out brief full-dark moments (e.g. shadow).
// END_ZONE_THRESHOLD   : normalised sensor value to count as "over black" (0-1000).
//                        500 works well — anything above half-range counts.
...
// Runtime state
unsigned long endZoneFirstSeen = 0;   // millis() when condition first met
bool          endZonePending   = false; // condition currently met but not confirmed yet
bool          endZoneTriggered = false; // latched true after confirmed stop
...
<div class="card" id="endZoneCard">
  ...
  <!-- Triggered banner (hidden by default) -->
  ...
  <div class="toggle-row" style="margin-bottom:10px">
    ...
    <span style="font-size:.75rem;color:#555;margin-left:6px">Stops when wide black square covers all sensors</span>
  </div>
  <div class="row">
  ...
  <!-- Live sensor coverage bar -->
  <div style="font-size:.7rem;color:#555;margin-top:4px;margin-bottom:4px">Live sensor coverage</div>
  <div style="display:flex;gap:4px" id="ezSensorDots">
  </div>
  <div style="font-size:.68rem;color:#444;margin-top:8px;line-height:1.6">
    <b style="color:#666">How it works:</b> The finish square covers all 6 sensors at once.
    On a normal line only 1–2 sensors are active — so ≥5 active simultaneously is unambiguous.
    The debounce timer prevents shadows or brief crossings from falsely stopping the bot.
  </div>
</div>
...
<div class="card">
  ...
  <div style="font-size:.68rem;color:#444;margin-top:6px;line-height:1.6">
    Detects a sharp corner the moment error <em>starts rising fast</em> — before it peaks.
    On trigger: speed cuts to <b style="color:#f59e0b">Corner Speed</b>, Kp multiplies by <b style="color:#7dd3fc">Boost Gain</b>,
    and pivot activates immediately. Badge above flashes <b style="color:#f59e0b">CORNER</b> in real time.<br>
    <b style="color:#666">Start with Sensitivity 400, Boost 2.5×, Duration 250ms, Speed 40%.</b>
  </div>
</div>
...
// Build end zone sensor dots on load
(function(){
...
function updateSensors(d){
  ...
  for(let i=0;i<6;i++){
    ...
    // End zone dots — green when sensor is above threshold
...
function sendTurn(){
  ...
  cmd('turn',{sharpness:sh, brake:br, curveExp:ce, turnSpeedReduce:sr,
              pivotThreshold:pt, cornerSensitivity:cs, cornerBoost:cb,
              cornerDuration:cd, cornerSpeedPct:csp});
}
...
// ══════════════════════════════════════════════════════════
//  PID  (with hairpin + 90° corner assist)
// ══════════════════════════════════════════════════════════
void runPID(){
  float currentError = (float)lastPosition;

  // ── Error derivative (smoothed) ──────────────────────────
  // How fast is the error changing this loop?
  // Smoothed with 0.4 low-pass to avoid single-sample noise spikes.
  float rawDeriv = currentError - pidError;   // pidError still holds last value
  errorDerivative = 0.6f * errorDerivative + 0.4f * rawDeriv;

  // ── Corner detection ─────────────────────────────────────
  // Trigger when error is growing fast AND we're not already in corner mode.
  if(CORNER_SENSITIVITY > 0 && !cornerActive
     && fabsf(errorDerivative) > (float)CORNER_SENSITIVITY){
    cornerActive  = true;
    cornerStartMs = millis();
    wsLog("[CORNER] detected dErr="+String((int)errorDerivative)
         +" err="+String((int)currentError));
  }
  // Expire corner mode after CORNER_DURATION_MS
  ...
  // ── Effective Kp — boosted during corner ─────────────────
  ...
  // ── 2. Effective base speed ───────────────────────────────
  // Two independent speed reductions:
  //   a) error-proportional slow (TURN_SPEED_REDUCE) — for hairpins
  //   b) corner-mode hard cut (CORNER_SPEED_PCT)     — for 90° corners
  ...
  if(cornerActive){
    ...
    effBase = min(effBase, cornerSpeed);   // take the harder cut
  }
  ...
  // ── 4. Pivot assist ──────────────────────────────────────
  // Normal pivot: error > PIVOT_THRESHOLD
  // Corner-mode pivot: always active when corner detected (ignores threshold),
  //   so 90° corners get pivot even before error reaches the normal threshold.
  ...
  if(doPivot){
    ...
    // During corner boost, depth starts at least 0.4 so pivot is immediate & strong
...
void onWsEvent(uint8_t num, WStype_t type, uint8_t* payload, size_t length){
  ...
  // ── Turn radius control ───────────────────────────────────
    ...
    wsLog("[TURN] sharp="+String(TURN_SHARPNESS,2)
         ...
         +" cornerSens="+String(CORNER_SENSITIVITY)
         +" boost="+String(CORNER_BOOST,1)
         +" dur="+String(CORNER_DURATION_MS)+"ms"
         +" cornerSpd="+String(CORNER_SPEED_PCT)+"%");
  }
  // ── End zone settings ────────────────────────────────────
    ...
    wsLog("[EZ] enable="+String(endZoneEnable)
         +" minSensors="+String(endZoneMinSensors)
         +" confirm="+String(endZoneConfirmMs)+"ms"
         +" thresh="+String(endZoneThreshold));
  }
...
//  End-zone detection
// ══════════════════════════════════════════════════════════
// Returns true the first time the end zone is confirmed.
// After that it is latched — call resetEndZone() to re-arm.
bool detectEndZone(){
  if(!endZoneEnable || !robotRunning) return false;

  // Count how many valid sensors are reading above the black threshold
  int active = 0;
  for(int i = 0; i < NUM_SENSORS; i++){
  ...
  if(conditionMet){
    if(!endZonePending){
      ...
      // Confirmed — stop the robot
      robotRunning     = false;
      endZoneTriggered = true;
      endZonePending   = false;
      stopMotors();
      wsLog("[END] Finish line detected! "+String(active)+" sensors active. Stopped.");
      String s = buildStateJson(); wsServer.broadcastTXT(s);
      return true;
    }
  } else {
    // Condition broke before debounce elapsed — reset pending
    endZonePending = false;
  }
  return false;
}
...
void loop(){
  ...
  if(calibrating){
    ...
    detectEndZone();   // check finish line BEFORE running PID
    if(robotRunning) runPID();  // only run if end zone didn't just stop us
  }
