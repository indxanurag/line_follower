...
<div class="card">
  ...
  <div style="display:flex;gap:8px;margin-top:4px;flex-wrap:wrap">
    <div id="turnPreview" style="
      position:relative;height:48px;flex:1;min-width:180px;
      background:#111;border:1px solid #2a2a2a;border-radius:8px;overflow:hidden">
      <canvas id="turnCanvas" width="400" height="48" style="width:100%;height:100%"></canvas>
    </div>
  </div>
  <div style="font-size:.68rem;color:#444;margin-top:6px">
    <b style="color:#666">Sharpness</b> scales PID output — raise for tight corners, lower for sweeping bends.<br>
    <b style="color:#666">Inner Brake</b> slows the inside wheel proportionally to error — most effective for hairpins.
  </div>
</div>
...
function updateState(d){
  ...
  if(typeof d.turnSharpness!=='undefined'){
    ...
    drawTurnPreview(d.turnSharpness, d.turnBrake);
  }
}
...
function sendTurn(){
  ...
  drawTurnPreview(sh, br);
  cmd('turn',{sharpness:sh,brake:br});
}
function drawTurnPreview(sharpness, brake){
  const canvas=document.getElementById('turnCanvas');
  if(!canvas) return;
  const ctx=canvas.getContext('2d');
  const W=canvas.width, H=canvas.height;
  ctx.clearRect(0,0,W,H);
  // Draw a simulated arc showing relative turn radius
  // Higher sharpness + higher brake = tighter arc
  const tightness = sharpness * (1 + brake/100);
  // radius inversely proportional to tightness, clamped so it stays visible
  const r = Math.max(18, Math.min(200, 120 / tightness));
  ctx.strokeStyle='#7dd3fc';
  ctx.lineWidth=2;
  ctx.beginPath();
  // Arc centred to the right of the canvas, curving left
  const cx=W-r, cy=H/2;
  const startAngle=Math.PI, endAngle=Math.PI+Math.PI*0.65;
  ctx.arc(cx, cy, r, startAngle, endAngle);
  ctx.stroke();
  // Direction arrow at end of arc
  const aX=cx+r*Math.cos(endAngle), aY=cy+r*Math.sin(endAngle);
  ctx.beginPath();
  ...
  ctx.fillText(tightness.toFixed(2)+'× effective',6,H-6);
}
</script>
</body>
</html>
...
void runPID(){
  ...
  // Apply turn sharpness multiplier — scales the whole correction
  float corr = rawCorr * TURN_SHARPNESS;

  int L = constrain(BASE_SPEED - (int)corr, MIN_SPEED, MAX_SPEED);
  int R = constrain(BASE_SPEED + (int)corr, MIN_SPEED, MAX_SPEED);

  // ── Turn brake ───────────────────────────────────────────
  // Extra braking on the inner (slower) wheel to tighten the radius.
  // The brake strength scales with |error| so it only kicks in on real curves.
  if(TURN_BRAKE > 0 && pidError != 0.0f){
    float normErr = constrain(fabsf(pidError) / 2500.0f, 0.0f, 1.0f); // 0..1
    float brakeScale = normErr * (TURN_BRAKE / 100.0f);
    if(pidError > 0){
      // turning right → left wheel is inner (slower)
      L = (int)(L - (L - MIN_SPEED) * brakeScale);
    } else {
      // turning left → right wheel is inner (slower)
      R = (int)(R - (R - MIN_SPEED) * brakeScale);
    }
...
void onWsEvent(uint8_t num, WStype_t type, uint8_t* payload, size_t length){
  ...
  // ── Turn radius control ───────────────────────────────────
    ...
    pidIntegral=0; // reset integral to avoid windup on param change
    wsLog("[TURN] sharpness="+String(TURN_SHARPNESS,2)+" brake="+String(TURN_BRAKE)+"%");
  }
